...
String str1="SELECT * FROM poduzece;";
...
float suma=0;
String imeprezime;
public blagajna(JButton buton1,String[] p,String[] k)
{
...
imeprezime=String.format("%s %s(%s)",p[1],p[2],p[0]);
labely4.setText(imeprezime);
...
t[2]="1";
//t[3]=rs.getString(4);
t[3]=String.format("%s",(rs.getDouble(4)+rs.getDouble(8))).replace(",",".");
t[4]=String.format("%.2f",(rs.getDouble(5)+rs.getDouble(9))).replace(",",".");;
t[5]=rs.getString(6);
t[6]="7";
...
try {
      ...
      stmt = c.createStatement();
String str1="SELECT count(*) FROM racun ;";
ResultSet rs = stmt.executeQuery(str1);
rs.next();

raci=rs.getInt(1);
System.out.println(raci);
...
String sqlxx= "insert into racun values ("+raci+",'"+Time+"','"+Date+"',"+suma+");";

stmt.executeUpdate(sqlxx);
...
c.close();
      ...
      System.err.println( e.getMessage() );
     JOptionPane.showMessageDialog(blagajna, "inicijalizacija");
//System.exit(0);
}
...
racunx=new SimpleDateFormat("yy");
racun=racunx.format(time);
String racunxy=String.format("%04d/",raci);

racunxy=racunxy+""+racun;

labely1.setText(Date);
labely2.setText(Time);
labely3.setText(racunxy);

String sumax=String.format("%.2f",suma);

new printer(model,Date,Time,imeprezime,sumax,racunxy,k);
...
String racunxy;
String [] k=null;
String xx0,xx1,xx2,xx3,xx4,xx5,xx6;
public printer(DefaultTableModel model,String Date,String Time,String imeprezime,String sumax,
String racunxy,String[] k) {
this.sumax=sumax;
this.k=k;
this.model=model;
this.Date=Date;
...
this.racunxy=racunxy;
System.out.printf("%s %s %s %s\n",Date,Time,imeprezime,racunxy);
System.out.printf("%s %s %s %s %s %s %s",k[0],k[1],k[2],k[3],k[4],k[5],k[6]);
...
public int print(Graphics g, PageFormat Pf, int pageIndex) throws PrinterException {
    ...
    g2d.setFont(new Font("Consolas",0,10));

g2d.drawString("=================", 0, 0);   
g2d.drawString(k[0], 0, 12);
g2d.drawString("vl."+k[1]+" "+k[2],0, 24);
g2d.drawString(k[3]+", "+k[4],0, 36);
g2d.drawString("OIB :"+k[5],0, 48);    
g2d.drawString("Tel. :"+k[6],0, 60);  

g2d.drawString("DATUM    :"+Date,0, 84);
g2d.drawString("VRIJEME  :"+Time,0, 96);
g2d.drawString("OPERATOR :"+imeprezime,0, 108);
g2d.drawString("RACUN    :"+racunxy,0, 120);
g2d.drawString("=============================",0,132);
g2d.setFont(new Font("Consolas",0,9));
g2d.drawString("Usluga ",0,140);
g2d.drawString("Kolicina ",95,140);
g2d.drawString("Mjera",0,150);
g2d.drawString("Cijena",35,150);
g2d.drawString("Pdv",75,150);
g2d.drawString("Iznos",110,150);
g2d.drawString("=============================",0,160);
int x=0;
for(int si=0;si<=model.getRowCount()-1;si++)
{

xx0=model.getValueAt(si, 0).toString();
xx1=model.getValueAt(si, 1).toString();
xx2=model.getValueAt(si, 2).toString();
xx3=model.getValueAt(si, 3).toString();
xx4=model.getValueAt(si, 4).toString();
xx5=model.getValueAt(si, 5).toString();
xx6=model.getValueAt(si, 6).toString();

g2d.drawString(xx1,0,170+x);
g2d.drawString(xx2,120,170+x);
g2d.drawString(xx3,0,180+x);
g2d.drawString(xx4,40,180+x);
g2d.drawString(xx5,75,180+x);
g2d.drawString(xx6,105,180+x);
x+=20;
}
g2d.drawString("=============================",0,170+x);

g2d.setFont(new Font("Consolas",0,12));
g2d.drawString("SUMA :",0,190+x);
g2d.drawString(sumax,80,190+x);
    return PAGE_EXISTS;
  }
